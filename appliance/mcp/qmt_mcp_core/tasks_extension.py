...
async def request_task_input(
    ...
    *,
    ...
    """Pause the current durable task until all standard MCP requests are answered."""

    interaction = _current_task_interaction.get()
    if interaction is None:
        raise RuntimeError("task input is only available during durable task execution")
...
class TasksExtension(Extension):
    ...
    async def intercept_tool_call(self, params: CallToolRequestParams, ctx, call_next):
        ...
        if params.name in self.mrtr_before_task_tools and not params.input_responses:
            return await call_next(ctx)
...
def register_task_conformance_fixtures(mcp) -> None:
    ...
    async def test_tool_with_task(ctx: Context) -> str | InputRequiredResult:
        ...
        value = to_jsonable_python(response, by_alias=True, exclude_none=True)
